...
impl<'b, T : Decoder<'b, Output=T> + 'b, S> List<T , S> {
    ...
    fn with_capacity(cap: usize) -> Self {
        List{ items:  SmallVec::with_capacity(cap), phantom2: PhantomData }
    }
...
impl<T, S> From<Vec<T>> for List<T , S> {
    ...
    fn from(vec: Vec<T>) -> Self {
        List{ items:  SmallVec::from(vec), phantom2: PhantomData }
    }
}
...
impl<T: Encoder, S: Encoder + Default> Encoder for List<T, S> where S: TryFrom<usize> {
    #[inline(always)]
    ...
    fn size_enc(&self) -> usize {
        ...
        self.items.iter().fold(S::size_enc(&len), |sum, x| sum + x.size_enc()) as usize
    }
}

impl<'b, T: Decoder<'b, Output=T> + 'b, S: Decoder<'b, Output=S> + Default> Decoder<'b> for List<T, S> where S::Output: Into<usize> {
    ...
    fn size(buffer: &'b [u8]) -> usize {
        let len = S::decode(buffer).into();
        (0..len).into_iter().fold(S::size(buffer), |sum, x| sum + T::size(&buffer[sum..])) as usize
    }
}
...
impl<'b, T, S: Clone> RefList<'b, T, S> {
    ...
    fn new(buffer: &'b[u8], len: S) -> Self {
        RefList{buffer: buffer,  len: len, phantom: PhantomData, phantom2: PhantomData}
    }

    #[inline(always)]
    pub fn iter(&self) -> RefList<'b, T, S> {
        RefList{buffer: self.buffer, len: self.len.clone(), phantom: self.phantom, phantom2: self.phantom2}
    }
}
...
impl<'b, T: Decoder<'b> + 'b, S: Decoder<'b, Output=S> + Default + Clone> Decoder<'b> for RefList<'b, T, S> where S::Output: Into<usize> {
